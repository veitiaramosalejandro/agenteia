...
from app.main import read_solidset_instance, read_solidset_instances


def instance_row(code="local-solidset", active=True):
    return {
        "ID": uuid4(), "Code": code, "Name": "SolidSET local",
        "BaseUrl": "http://localhost:52130",
        "NotificationUrl": "http://localhost:52131", "SourceIP": "localhost",
        "CountryCode": "PT", "Locale": "pt-PT", "TimeZone": "Europe/Lisbon",
        "active": active, "CreatedAt": datetime.now(timezone.utc),
        "UpdatedAt": datetime.now(timezone.utc),
        "DataAPI": {
            "BaseUrl": "http://solidset-data-api:8080",
            "EncryptedAPIKey": "must-not-leak", "TimeoutSeconds": 120,
            "MaxRows": 5000, "VerifyTLS": False, "active": True,
        },
    }


class TestSolidSETInstancesRead(unittest.TestCase):
    @patch("app.main.list_active_solidset_instances")
    ...
    def test_get_one_includes_inactive_and_hides_credentials(self, get_instance):
        ...
        get_instance.assert_called_once_with(
            code="inactive", source_ip=None, active_only=False,
        )
        self.assertEqual("inactive", result.Code)
        self.assertFalse(result.active)
        self.assertTrue(result.DataAPI.APIKeyConfigured)

    @patch("app.main.get_solidset_instance", return_value=None)
