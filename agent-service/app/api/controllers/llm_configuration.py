...
@router.put("/api/v1/agent/llm/providers/{code}", response_model=LLMProviderConfigurationResponse)
def save_llm_provider(
    request: Request, code: str, configuration: LLMProviderConfiguration,
) -> LLMProviderConfigurationResponse:
    ...
    if provider not in ProviderRegistry.names():
        raise HTTPException(status_code=422, detail={
            "message": "Fornecedor LLM não suportado.", "available": list(ProviderRegistry.names()),
        })
    payload.update(Code=payload["Code"].strip(), Name=payload["Name"].strip(),
                   Provider=provider, Model=payload["Model"].strip())
...
@router.get("/api/v1/agent/llm/providers", response_model=list[LLMProviderConfigurationStored])
...
@router.put("/api/v1/agent/solidset/agents/{agent_resource_id}/model", response_model=AgentIAModelStored)
def configure_agent_model(
    request: Request, agent_resource_id: UUID, configuration: AgentIAModelConfiguration,
) -> AgentIAModelStored:
    payload = configuration.model_dump()
