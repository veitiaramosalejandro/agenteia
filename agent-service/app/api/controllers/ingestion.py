...
from app.connectors.db_client import get_solidset_instance, list_active_solidset_instances
...
@router.post("/api/v1/agent/system-knowledge-ingestion/start", status_code=202, dependencies=admin)
...
def system_knowledge_ingestion_status(
    ...
    try:
        result = get_run_status(run_id=str(runId) if runId else None, instance_id=instance_id)
...
@router.post("/api/v1/agent/historical-ingestion/start", status_code=202, dependencies=admin)
async def start_historical_ingestion(configuration: HistoricalIngestionStartRequest) -> dict[str, Any]:
    try:
        instances = ([get_solidset_instance(code=configuration.instanceCode, source_ip=None)]
        ...
        results = [await asyncio.to_thread(enqueue_next_batch, item, configuration.dryRun)
...
def approve_historical_dry_run(instanceCode: str = Query(...)) -> dict[str, Any]:
    ...
    return {"status": "approved", "approvedCursors": approve_dry_run_cursors(str(instance["ID"]))}


@router.get("/api/v1/agent/historical-ingestion/status", dependencies=admin)
def historical_ingestion_status(resourceId: Optional[UUID] = Query(None)) -> dict[str, Any]:
    return {"queue": historical_queue.stats(),
...
def historical_ingestion_batches(
    limit: int = Query(50, ge=1, le=500), resourceId: Optional[UUID] = Query(None),
) -> dict[str, Any]:
    return {"items": list_audits(limit, str(resourceId) if resourceId else None)}


@router.delete("/api/v1/agent/historical-ingestion/messages/{id_chat2}", dependencies=admin)
def delete_historical_message(
    ...
    return {"status": "deleted", "idChat2": id_chat2,
