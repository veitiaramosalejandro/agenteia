...
from app.historical.store import (
    ensure_schema, save_document, set_cursor, upsert_audit, workroom_agents,
)
...
def _document(row: dict[str, Any], instance_id: str, scope: str, agent: dict[str, Any]) -> dict[str, Any]:
    ...
    room_id = str(row["IDWorkRoom"])
    key = f"solidset:{instance_id}:chat:{row['IDChat2']}:{scope}:{resource_id}:{room_id}"
    ...
    return {
        ...
        "IDWorkRoom": uuid.UUID(room_id), "ContentHash": row["ContentHash"],
        "text": f"{sender_name}: {row['NormalizedText']}",
        "payload": {
            "page_content": f"{sender_name}: {row['NormalizedText']}",
            "document_type": "historical_message", "source": "solidset_sql_history",
            "solidset_instance_id": instance_id, "id_chat2": int(row["IDChat2"]),
...
def process_batch(batch: dict[str, Any]) -> dict[str, int]:
    ...
    for raw in batch.get("messages") or []:
        ...
        if reason:
            rejected += 1; continue
        agents = workroom_agents(str(row["IDWorkRoom"]))
        if not agents:
            rejected += 1; continue
        for agent in agents:
            scope = "owner" if str(agent["IDResource"]).lower() == str(row["IDSenderResource"]).lower() else "workroom"
            documents.append(_document(row, batch["instanceId"], scope, agent))
        accepted += 1
    if batch.get("dryRun"):
        upsert_audit(batch, "dry_run", accepted=accepted, rejected=rejected, indexed=0)
        set_cursor(batch["instanceId"], batch["firstIdChat2"] - 1, None, "dry_run")
    ...
    set_cursor(batch["instanceId"], int(batch["lastIdChat2"]), last.get("Stamp"), "completed")
        ...
        for message_id, fields in messages:
            ...
            try:
                ...
                if attempt < settings.HISTORICAL_INGESTION_MAX_RETRIES:
                    ...
                    set_cursor(batch["instanceId"], batch["firstIdChat2"]-1, None, "failed", str(exc))
                await asyncio.to_thread(queue.ack, message_id)
