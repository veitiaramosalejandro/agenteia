      ...
      <main>
        <header className="topbar">
          ...
          <div className="instance-picker">
            ...
            <div className="select-wrap"><SearchSelect value={instanceCode} onChange={setInstanceCode} ariaLabel="Instancia activa" placeholder="Selecciona una instancia" searchPlaceholder="Buscar instancia…" options={instances.map(item => ({ value: item.Code, label: item.Name, detail: item.Code }))} /></div>
          </div>
...
function Prompts({ selectedInstance, agents, notify, refreshAgents }: Shared) {
  ...
  useEffect(() => {
    let cancelled = false
    const selected = agents.find(agent => agent.IDResource === agentId)
    if (!selectedInstance || !selected) { setDraft(null); return }
    if (!selected.prompt) { setDraft(null); setForm(defaultPrompt); return }
    setBusy(true)
    void api.publishedPrompt(agentId, selectedInstance.Code)
      .then(prompt => {
        if (cancelled) return
        const behavior = prompt.BehaviorConfig || {}
        const list = (key: string) => Array.isArray(behavior[key]) ? (behavior[key] as string[]).join('\n') : ''
        setForm({
          name: prompt.Name || defaultPrompt.name,
          role: String(behavior.role || defaultPrompt.role),
          objective: String(behavior.objective || defaultPrompt.objective),
          specialties: list('specialties'),
          code_review_instructions: list('code_review_instructions'),
          response_format: list('response_format'),
          restrictions: list('restrictions'),
          out_of_scope_action: String(behavior.out_of_scope_action || ''),
          tone: String(behavior.tone || defaultPrompt.tone),
          response_style: String(behavior.response_style || defaultPrompt.response_style),
          default_language: String(behavior.default_language || defaultPrompt.default_language),
          created_by: defaultPrompt.created_by,
        })
        setDraft(prompt)
      })
      .catch(error => { if (!cancelled) { setDraft(null); notify('error', error instanceof Error ? error.message : 'No fue posible cargar el prompt publicado.') } })
      .finally(() => { if (!cancelled) setBusy(false) })
    return () => { cancelled = true }
  }, [agentId, agents, selectedInstance, notify])
...
function Models({ selectedInstance, agents, providers, notify, refreshAgents }: Shared) {
  ...
  return <><PageHeader eyebrow="Enrutamiento" title="Modelos y capacidades" copy="Asigna conexiones reutilizables a un agente dentro de la instancia seleccionada." />
    <div className="split-grid model-layout"><section className="panel"><span className="eyebrow">Conexiones disponibles</span><h2>Proveedores LLM</h2><div className="provider-list">{providers.map(p => <div key={p.ID}><div className={`provider-dot ${p.active ? 'on' : ''}`} /><div><strong>{p.Name}</strong><small>{p.Provider} · {p.Model}</small></div>{p.IsDefault && <span className="status-pill">Global</span>}</div>)}</div></section><section className="panel form-panel"><span className="eyebrow">Nueva asignación</span><h2>Configurar agente</h2><label>Agente<SearchSelect value={agentId} onChange={setAgentId} ariaLabel="Agente" searchPlaceholder="Buscar agente…" options={agents.map(a => ({ value: a.IDResource, label: a.Name, detail: a.FullName || shortId(a.IDResource) }))} /></label><label>Conexión<SearchSelect value={providerCode} onChange={setProviderCode} ariaLabel="Conexión LLM" searchPlaceholder="Buscar conexión o modelo…" options={providers.filter(p => p.active).map(p => ({ value: p.Code, label: p.Name, detail: `${p.Provider} · ${p.Model}` }))} /></label><div className="form-row"><Field label="Rol" value={role} onChange={setRole} /><label>Prioridad<input type="number" value={priority} onChange={e => setPriority(Number(e.target.value))} min="0" max="10000" /></label></div><label>Capacidades</label><div className="chips">{['general', 'coding', 'reasoning', 'sql', 'external_web'].map(cap => <button key={cap} className={capabilities.includes(cap) ? 'selected' : ''} onClick={() => toggle(cap)}>{cap}</button>)}</div><button className="primary" onClick={() => void save()} disabled={busy || !selectedInstance}><Save size={17} /> Guardar asignación</button></section></div>
  </>
}
...
function Knowledge({ selectedInstance, agents, notify }: Shared) {
  ...
  const agentPicker = <label>Agente<SearchSelect value={agentId} onChange={value => { setAgentId(value); setResult(null) }} ariaLabel="Agente" searchPlaceholder="Buscar agente…" options={agents.map(a => ({ value: a.IDResource, label: a.Name, detail: a.FullName || shortId(a.IDResource) }))} /></label>
...
function Observability({ selectedInstance, agents, notify }: Shared) {
  ...
  return <>
    ...
    <section className="observability-filters panel"><label>Ventana<select value={hours} onChange={e => setHours(Number(e.target.value))}><option value="1">Última hora</option><option value="24">24 horas</option><option value="168">7 días</option><option value="720">30 días</option></select></label><label>Tipo<select value={eventType} onChange={e => setEventType(e.target.value)}><option value="">Todos</option><option value="response">Respuestas</option><option value="automation">Automatización</option><option value="ingestion">Ingestión</option><option value="tool">Herramientas</option></select></label><label>Estado<select value={status} onChange={e => setStatus(e.target.value)}><option value="">Todos</option><option value="completed">Completado</option><option value="failed">Fallido</option><option value="blocked">Bloqueado</option><option value="queued">En cola</option><option value="running">En ejecución</option></select></label><label>Agente<SearchSelect value={resourceId} onChange={setResourceId} ariaLabel="Filtrar por agente" placeholder="Todos" searchPlaceholder="Buscar agente…" options={[{ value: '', label: 'Todos los agentes' }, ...agents.map(agent => ({ value: agent.IDResource, label: agent.Name, detail: shortId(agent.IDResource) }))]} /></label><label className="inline-check"><input type="checkbox" checked={autoRefresh} onChange={e => setAutoRefresh(e.target.checked)} /> Actualizar cada 15 s</label></section>
...
function SearchSelect({ value, options, onChange, placeholder = 'Seleccionar…', searchPlaceholder = 'Buscar…', ariaLabel }: { value: string; options: SelectOption[]; onChange: (value: string) => void; placeholder?: string; searchPlaceholder?: string; ariaLabel: string }) {
  ...
  useEffect(() => { if (open) { setQuery(''); setActive(0); window.setTimeout(() => input.current?.focus(), 0) } }, [open])
  const choose = (option: SelectOption) => { onChange(option.value); setOpen(false); setQuery('') }
  const keyDown = (event: React.KeyboardEvent) => {
    if (!open && ['Enter', ' ', 'ArrowDown'].includes(event.key)) { event.preventDefault(); setOpen(true); return }
    if (!open) return
    if (event.key === 'Escape') { setOpen(false); return }
    if (event.key === 'ArrowDown') { event.preventDefault(); setActive(index => filtered.length ? Math.min(index + 1, filtered.length - 1) : 0) }
    if (event.key === 'ArrowUp') { event.preventDefault(); setActive(index => Math.max(index - 1, 0)) }
    if (event.key === 'Enter' && filtered[active]) { event.preventDefault(); choose(filtered[active]) }
  }
  return <div className={`smart-select ${open ? 'open' : ''}`} ref={root} onKeyDown={keyDown}>
    <button type="button" className="smart-select-trigger" aria-label={ariaLabel} aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(state => !state)}><span>{selected?.label || placeholder}{selected?.detail && <small>{selected.detail}</small>}</span><ChevronDown size={16} /></button>
    {open && <div className="smart-select-menu"><div className="smart-select-search"><Search size={15} /><input ref={input} value={query} onChange={event => { setQuery(event.target.value); setActive(0) }} placeholder={searchPlaceholder} aria-label={searchPlaceholder} /></div><div className="smart-select-options" role="listbox">{filtered.length ? filtered.map((option, index) => <button type="button" role="option" aria-selected={option.value === value} className={`${index === active ? 'active' : ''} ${option.value === value ? 'selected' : ''}`} key={option.value} onMouseEnter={() => setActive(index)} onClick={() => choose(option)}><span>{option.label}<small>{option.detail}</small></span>{option.value === value && <CheckCircle2 size={15} />}</button>) : <div className="smart-select-empty">Sin coincidencias</div>}</div></div>}
  </div>
}

function Field({ label, value, onChange, area = false, maxLength }: { label: string; value: string; onChange: (value: string) => void; area?: boolean; maxLength?: number }) { return <label>{label}{area ? <textarea value={value} maxLength={maxLength} onChange={e => onChange(e.target.value)} rows={3} /> : <input value={value} maxLength={maxLength} onChange={e => onChange(e.target.value)} />}{maxLength && <small className="field-counter">{value.length}/{maxLength}</small>}</label> }
